angular.module('moviesowlApp').run(['$templateCache', function($templateCache) {
  ...
  $templateCache.put('templates/showings.html',
    ...
    "                    </div>\n" +
    "                </div>\n" +
    "            </div> --><ion-list><ion-item class=\"ratingCell\"><div>Rotten Tomatoes</div><img src=\"{{rottenLogo}}\" style=\"width:25px\"> {{movie.tomato_meter}}%</ion-item><ion-item class=\"item-icon-right item-dark\" ng-click=\"openModal()\" style=\"background-color: #222\"><!--{{ movie.synopsis }}-->More Info <i class=\"icon ion-chevron-right icon-accessory\"></i></ion-item><div class=\"item item-dark item-divider\">Today's Sessions</div><ion-item ng-repeat=\"session in showingsData\" ng-click=\"openSeatView(session.id)\" class=\"sessionsCell item-dark\" style=\"background-color: #222\"><div class=\"row\"><div class=\"col col-25\" style=\"text-align:left; font-weight:bold\">{{session.start_time*1000| date:'h:mma' }}</div><div class=\"col col-35\" ng-show=\"session.screen_type === 'standard'\"><span ng-show=\"session.showing_type ==='3D'\" style=\"color:red; font-weight:bold\">3D</span> {{session.cinemaSize}}</div><div class=\"col col-35\" ng-hide=\"session.screen_type === 'standard'\"><span ng-show=\"session.showing_type ==='3D'\" style=\"color:red; font-weight:bold\">3D</span> <span ng-show=\"session.screen_type ==='gold class'\" style=\"color:rgb(255, 237, 54); font-weight:bold\">Gold Class</span> <span ng-show=\"session.screen_type ==='vmax'\" style=\"color:rgb(68, 156, 255); font-weight:bold\">VMAX</span></div><div class=\"col col-40\" style=\"text-align:right\" ng-show=\"session.fullness.length > 0\">{{session.fullness}}</div><div class=\"col col-40\" style=\"text-align:right\" ng-hide=\"session.fullness.length > 0\"><div class=\"custom-spinner\"><div class=\"dot1\"></div><div class=\"dot2\"></div></div></div></div></ion-item></ion-list></div></ion-content></ion-view>"
  );


  $templateCache.put('templates/times.html',
    "<ion-modal-view><ion-header-bar><h1 class=\"title\">Starting After</h1><button class=\"button button-outline button-dark\" ng-click=\"closeModalOnly()\">Done</button></ion-header-bar><ion-content><ion-item ng-repeat=\"time in times\" ng-click=\"setStartingTime(time)\">{{ time | date:'shortTime' }}</ion-item><!--<label ng-repeat=\"cinema in cinemas\" class=\"item item-radio item-icon-right\" ng-click=\"closeModal(cinema)\">--><!--<input type=\"radio\" name=\"group\">--><!--<div class=\"item-content\">--><!--<span>{{cinema.location}}</span>--><!--<span ng-show=\"cinemaLocation === cinema.location\">--><!--<i class=\"icon ion-ios-heart icon-accessory\" style=\"color:deeppink; font-size:30px; width:60px;\"></i> --><!--</span>--><!--<span ng-hide=\"cinemaLocation === cinema.location\">--><!--<i class=\"icon ion-ios-heart-outline icon-accessory\" style=\"font-size: 30px; width:60px;\"></i>--><!--</span>--><!--</div>--><!--</label>--></ion-content></ion-modal-view>"
  );
